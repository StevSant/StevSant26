  ...
  template: `
    <div class="space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-white">Competencias</h1>
          <p class="text-gray-400 mt-1">Gestiona tus competencias y hackathons</p>
        </div>
      ...
      @if (loading()) {
        ...
        <div class="bg-gray-800 rounded-xl p-12 text-center">
          ...
          <h3 class="mt-4 text-lg font-medium text-white">No hay competencias</h3>
          <p class="mt-2 text-gray-400">Comienza agregando una competencia</p>
          ...
          @for (item of filteredItems(); track item.id) {
            <div cdkDrag [cdkDragDisabled]="showArchived()" class="bg-gray-800 rounded-lg p-4 flex items-center gap-4 group hover:bg-gray-750 transition-colors" [class.opacity-60]="item.is_archived">
              ...
              <div class="flex-1 min-w-0">
                ...
                @if (item.organizer) { <p class="text-gray-400 text-sm truncate">{{ item.organizer }}</p> }
                <div class="flex items-center gap-2 mt-1">
                  @if (item.date) { <span class="text-gray-500 text-xs">{{ item.date | date:'mediumDate' }}</span> }
                  @if (item.result) {
                    <span class="px-2 py-0.5 text-xs rounded-full bg-green-500/20 text-green-400">{{ item.result }}</span>
                  }
                </div>
              </div>
      ...
      <app-confirm-dialog #confirmDialog title="Eliminar competencia" message="¿Estás seguro de que deseas eliminar esta competencia?" confirmText="Eliminar" (confirmed)="deleteItem()"/>
    </div>
  `,
})
