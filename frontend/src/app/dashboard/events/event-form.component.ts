  ...
  template: `
    <div class="max-w-2xl mx-auto">
      <div class="flex items-center gap-4 mb-6">
        <a routerLink="/dashboard/events" class="p-2 text-gray-400 hover:text-white hover:bg-gray-700 rounded-lg transition-colors">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
          </svg>
        </a>
        <h1 class="text-2xl font-bold text-white">{{ isNew ? 'Nuevo Evento' : 'Editar Evento' }}</h1>
      </div>
      ...
      @if (loading()) {
        ...
        <form (ngSubmit)="onSubmit()" class="space-y-6">
          <div class="bg-gray-800 rounded-xl p-6 space-y-6">
            ...
            <div>
              <label for="assisted_at" class="block text-sm font-medium text-gray-300 mb-2">Fecha de Asistencia</label>
              <input type="date" id="assisted_at" name="assisted_at" [(ngModel)]="formData.assisted_at"
          ...
          <div class="bg-gray-800 rounded-xl p-6 space-y-6">
            <h2 class="text-lg font-semibold text-white">Imágenes del Evento</h2>
            <app-image-upload folder="events" [multiple]="true" [sourceType]="'event'" [sourceId]="currentId" (uploaded)="onImageUploaded($event)"/>
          </div>
          ...
          <div class="flex justify-end gap-3">
            <a routerLink="/dashboard/events" class="px-6 py-2.5 bg-gray-700 hover:bg-gray-600 text-white font-medium rounded-lg transition-colors">Cancelar</a>
            <button type="submit" [disabled]="saving()" class="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-600/50 disabled:cursor-not-allowed text-white font-medium rounded-lg transition-colors flex items-center gap-2">
              @if (saving()) {
                ...
                <span>{{ isNew ? 'Crear Evento' : 'Guardar Cambios' }}</span>
              }
            </button>
          </div>
        </form>
