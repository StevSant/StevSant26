    ...
    <div class="space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="text-gray-400 mt-1">Gestiona tu experiencia profesional</p>
        </div>
      ...
      @if (loading()) {
        ...
        <div class="bg-gray-800 rounded-xl p-12 text-center">
          ...
          <h3 class="mt-4 text-lg font-medium text-white">No hay experiencias</h3>
          <p class="mt-2 text-gray-400">Comienza agregando tu experiencia profesional</p>
          ...
          @for (item of filteredItems(); track item.id) {
            <div cdkDrag [cdkDragDisabled]="showArchived()" class="bg-gray-800 rounded-lg p-4 flex items-center gap-4 group hover:bg-gray-750 transition-colors" [class.opacity-60]="item.is_archived">
              ...
              <div class="flex-1 min-w-0">
                <h3 class="text-white font-medium truncate">{{ item.role }}</h3>
                <p class="text-gray-400 text-sm truncate">{{ item.company }}</p>
                <p class="text-gray-500 text-xs mt-1">
                  {{ item.start_date | date:'MMM yyyy' }}
                  @if (item.end_date) { - {{ item.end_date | date:'MMM yyyy' }} }
                  @else { - Presente }
                </p>
              </div>
      ...
      <app-confirm-dialog #confirmDialog title="Eliminar experiencia" message="¿Estás seguro de que deseas eliminar esta experiencia?" confirmText="Eliminar" (confirmed)="deleteItem()"/>
    </div>
  `,
})
export class ExperiencesComponent implements OnInit {
  ...
  async loadItems(): Promise<void> {
    ...
    try {
      const { data, error } = await this.supabase.getAll<Experience>('experience', 'position', true);
