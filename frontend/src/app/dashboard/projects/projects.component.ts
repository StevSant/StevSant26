    ...
    <div class="space-y-6">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p class="text-gray-400 mt-1">Gestiona tus proyectos del portafolio</p>
        </div>
      ...
      @if (loading()) {
        ...
        <div class="bg-gray-800 rounded-xl p-12 text-center">
          ...
          <h3 class="mt-4 text-lg font-medium text-white">No hay proyectos</h3>
          <p class="mt-2 text-gray-400">Comienza creando tu primer proyecto</p>
          <a
            routerLink="new"
            class="mt-6 inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors"
          >
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
            </svg>
          ...
          @for (item of filteredItems(); track item.id) {
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                @if (item.url) {
                  <a [href]="item.url" target="_blank" class="text-blue-400 text-sm hover:underline mt-1 inline-flex items-center gap-1">
              ...
              <div class="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button
                  ...
                  class="p-2 text-gray-400 hover:text-yellow-500 hover:bg-gray-700 rounded-lg transition-colors"
                >
                ...
                <a
                  [routerLink]="[item.id]"
                  class="p-2 text-gray-400 hover:text-blue-500 hover:bg-gray-700 rounded-lg transition-colors"
                  title="Editar"
                >
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  ...
                  [title]="item.is_archived ? 'Restaurar' : 'Archivar'"
                  class="p-2 text-gray-400 hover:text-orange-500 hover:bg-gray-700 rounded-lg transition-colors"
                >
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    @if (item.is_archived) {
                ...
                <button
                  ...
                  class="p-2 text-gray-400 hover:text-red-500 hover:bg-gray-700 rounded-lg transition-colors"
                  title="Eliminar"
                >
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export class ProjectsComponent implements OnInit {
  ...
  async loadItems(): Promise<void> {
    ...
    try {
      const { data, error } = await this.supabase.getAll<Project>('project', 'position', true);
