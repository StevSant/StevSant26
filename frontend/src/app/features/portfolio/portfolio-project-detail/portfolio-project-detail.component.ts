          ...
          <article>
            ...
            <div class="mb-8">
              <div class="flex items-start gap-3 mb-2">
                <h1 class="text-3xl font-semibold text-(--color-text-primary) tracking-tight">
                  {{ data.getEntityTranslation(p, 'title') }}
                </h1>
                @if (p.is_pinned) {
                  <span class="text-(--color-pin) mt-1">
              ...
              @if (p.created_at) {
                <p class="text-xs text-(--color-text-muted) uppercase tracking-wider mb-4">
                  {{ p.created_at | date: 'longDate' }}
                </p>
              }
              @if (p.url) {
                <a [href]="p.url" target="_blank" class="inline-flex items-center gap-1.5 text-sm text-(--color-accent) hover:text-(--color-accent-hover) transition-colors">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div class="mb-10">
                ...
                <div class="relative rounded-lg overflow-hidden mb-3 bg-(--color-bg-tertiary)">
                  <img
                    ...
                    [alt]="images()[selectedImageIndex()].alt_text || data.getEntityTranslation(p, 'title')"
            ...
            <!-- Description -->
            @if (data.getEntityTranslation(p, 'description')) {
              <div class="mb-10">
                <p class="text-(--color-text-secondary) leading-relaxed whitespace-pre-line">
                  {{ data.getEntityTranslation(p, 'description') }}
                </p>
              </div>
            }

            <!-- Skills Used -->
            @if (data.getSkillUsages('project', p.id).length > 0) {
              <div class="mb-10">
                <h2 class="text-xs uppercase tracking-[0.2em] text-(--color-text-muted) mb-4 font-medium">
                  {{ 'portfolio.skillsUsed' | translate }}
                </h2>
                <div class="flex flex-wrap gap-2">
                  @for (usage of data.getSkillUsages('project', p.id); track usage.id) {
