    ...
    <section class="py-24 px-4 sm:px-6 lg:px-8">
      <div class="max-w-6xl mx-auto">
        <h2 class="text-sm uppercase tracking-[0.3em] text-(--color-text-muted) mb-3 text-center font-medium">
          {{ 'portfolio.sections.skills' | translate }}
        </h2>
        ...
        @if (data.skillCategories().length === 0) {
          <p class="text-(--color-text-muted) text-center italic font-light">
            {{ 'portfolio.empty.skills' | translate }}
          </p>
        } @else {
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            @for (category of data.skillCategories(); track category.id) {
              <div class="bg-(--color-bg-secondary) rounded-lg p-6 border border-(--color-card-border)">
                <h3 class="text-sm uppercase tracking-[0.2em] text-(--color-accent) mb-6 font-medium">
                  {{ data.getCategoryName(category) }}
                </h3>
                <div class="space-y-4">
                  @for (skill of category.skills; track skill.id) {
                    <div class="flex items-center justify-between">
                      <span class="text-(--color-text-primary) text-sm font-light">
                        {{ data.getEntityTranslation(skill, 'name') }}
                      </span>
                      <div class="flex gap-1">
                        @for (filled of data.getStarArray(skill.calculatedLevel); track $index) {
                          <svg
                            class="w-4 h-4"
                            [class.text-(--color-accent)]="filled"
                            [class.text-(--color-border-primary)]="!filled"
                            fill="currentColor"
                            viewBox="0 0 20 20"
                          >
