...
export class SkillUsageListComponent implements OnInit {
  ...
  async loadItems(): Promise<void> {
    ...
    try {
      const { data, error } =
        await this.translationDataService.getSkillUsagesWithSkills<SkillUsage>();
      if (error) throw error;
      this.items.set(data || []);
    } catch (err) {
      this.logger.error('Error loading skill usages:', err);
    } finally {
  ...
  async drop(event: CdkDragDrop<SkillUsage[]>): Promise<void> {
    ...
    try {
      await this.crudService.updatePositions('skill_usages', updates);
    } catch (err) {
  ...
  async toggleArchive(item: SkillUsage): Promise<void> {
    try {
      if (item.is_archived) {
        await this.crudService.unarchive('skill_usages', item.id);
      } else {
        await this.crudService.archive('skill_usages', item.id);
      }
      await this.loadItems();
    } catch (err) {
  ...
  confirmDelete(item: SkillUsage): void {
  ...
  async deleteItem(): Promise<void> {
    ...
    try {
      await this.crudService.delete('skill_usages', this.itemToDelete.id);
      await this.loadItems();
    } catch (err) {
